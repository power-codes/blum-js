...
class AuthService {
  ...
  async handleLogin(user) {
    ...
    user.log.logError(
        "The login process failed, please check your account information again (may need to get new query_id).The system will try to log in again after 60 seconds"
    );
    return {
      status: 0,
      profile: null,
  ...
  async getProfile(user) {
    try {
      ...
      user.log.logError(
        `Retrieve account information failed: ${error.response?.data?.message}`
      );
      return null;
    }
  }
